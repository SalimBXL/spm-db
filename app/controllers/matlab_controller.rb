class MatlabController < ApplicationController
    ...
    def add_pdf_to_db_ok
        ...
        if spm.size < 1
            ...
            # save 
            if (spms.length == 2)
                base = File.join('/Database', session[:patient_id], session[:study_date], @file_spm_base)
                mirror = File.join('/Database', session[:patient_id], session[:study_date], @file_spm_mirror)
